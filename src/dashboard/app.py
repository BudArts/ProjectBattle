...
with tab_over:
    ...
    st.caption(
        "Прогон: 43 состава с 1 апреля 2028, горизонт 365 суток, "
        "начальное значение генератора 42, сценарий «нормальная эксплуатация», "
        "планирование включено. Реактивный режим — на соседней вкладке."
    )
    if plan:
...
with tab_fleet:
    ...
    trips_path = LAST / "plan_normal_hourly.csv"
    st.caption(
        "Таблица читает текущую базу. После прогона на вкладке «Симуляция» "
        "счётчики совпадут с последним часом модели. "
        "Пилу пробега строит живой прогон: в сохранённом годе есть интервалы работ, "
        "а не каждый полурейс."
    )
...
with tab_sim:
    ...
    st.write(
        "Сохранённый год уже посчитан и показан на остальных вкладках. "
        "Здесь можно пересчитать другой горизонт. Полный год с планировщиком "
        "занимает около двух минут: решатель годового плана работает до 30 секунд."
    )
    c1, c2, c3, c4 = st.columns(4)
    years = c1.slider("Горизонт, лет", 0.25, 1.0, 0.25, 0.25)
    ...
    if st.button("Запустить симуляцию", type="primary"):
        ...
        st.success(
            ...
            f"внеплановых событий {results['total_breakdowns']}, "
...
# ================================================================ СТАНОК
with tab_lathe:
    st.subheader("Когда одного станка мало")
    ...
    kpi_row([
        ...
        ("Дневная обточка без плана",
         ...
         "тот же парк, реактивный заход"),
    ])
    st.caption(
        "Доли узлов отказа — рабочая гипотеза: заказчик статистику не передал. "
        "В модели колёса и тележки 28%, тормоз 16%, токоприёмник, тяга и двери по 12%. "
        "Обточка и замена колёсной пары — разные работы: обточка только в Обухово, "
        "замену можно закрыть в Москве."
    )
    if plan:
        ...
        if nodes:
            ...
            st.plotly_chart(
                base_layout(fig, "Внеплановые события по узлам за год с планированием",
                            360, "Узел", "Событий"),
                width="stretch")
